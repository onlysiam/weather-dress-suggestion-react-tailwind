...
const Resetpass = ({
  ...
  const mailHandler = (e) => {
    setpreloader(true);
    e.preventDefault();
    Axios.post("https://weathercloset.onlysiam.com/api/mailchecker", {
      email: mail,
    }).then((response) => {
      console.log(response);
      if (response.data.length > 0) {
        Axios.post("https://weathercloset.onlysiam.com/api/resetreq", {
          usermail: mail,
          email: mail,
        }).then((response) => {
  ...
  const otpHandler = (e) => {
    setpreloader(true);
    e.preventDefault();
    Axios.post("https://weathercloset.onlysiam.com/api/otpchecker", {
      email: mail,
      otp: otp,
    }).then((response) => {
      if (response.data.length > 0) {
  ...
  const resetHandler = (e) => {
    ...
    if (newpass === newpassVerify) {
      Axios.post("https://weathercloset.onlysiam.com/api/resetpass", {
        email: mail,
        pass: newpassVerify,
      }).then((response) => {
        console.log(response.data);
